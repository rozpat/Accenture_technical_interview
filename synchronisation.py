...
def sync_vicon_treadmill(df_vicon, df_treadmill, v, name):
    ...
    The function returns the median time difference between vicon and treadmill peaks occurence
    and plots the results if v = True '''
    try:
        ...
        treadmill_peak_timestamps = np.array(df_treadmill['timestamp'].iloc[peaks])
        ...
        time_intervals = np.diff(vicon_minima_timestamps)
        ...
        # MEDIAN
        time_diff_treadmill_vicon_median = "{:.3f}".format(np.median(time_diff_treadmill_vicon))
        # Change str to float
...
def sync_vicon_acc(accDF, viconDF, start_time, end_time, plot=True):
    ...
    # Time difference TREADMILL - VICON
    time_diff_acc_vicon = (acc_peaks_timestamps - matching_timestamps_filtered)
    # MEDIAN
    time_diff_acc_vicon_median = "{:.2f}".format(np.median(time_diff_acc_vicon))
    # Change str to float
